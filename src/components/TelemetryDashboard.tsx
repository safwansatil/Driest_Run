...
export const TelemetryDashboard: React.FC = () => {
  ...
  return (
    <div className="glass-panel" style={{ width: '320px', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', zIndex: 10 }}>
      
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '0.5rem' }}>
        <Activity size={20} color="#00ccff" />
        <h2 style={{ margin: 0, fontSize: '1.2rem', color: '#fff' }}>Live Telemetry</h2>
      </div>

      {/* Cartesian Pose */}
      <div>
        <h3 style={{ margin: '0 0 0.75rem 0', fontSize: '0.9rem', color: '#00ccff', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      <div>
        <h3 style={{ margin: '0 0 0.75rem 0', fontSize: '0.9rem', color: '#ffb732' }}>Joint Angles (rad)</h3>
            ...
            return (
              <div key={name} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', fontFamily: 'monospace' }}>
                  ...
                  <span style={{ color: isNearLimit ? '#ff4444' : '#fff' }}>{value.toFixed(3)}</span>
                </div>
                <div style={{ width: '100%', height: '4px', background: 'rgba(0,0,0,0.5)', borderRadius: '2px', overflow: 'hidden' }}>
                  <div 
                    style={{ 
                      width: `${percentage}%`, 
                      height: '100%', 
                      background: isNearLimit ? '#ff4444' : '#ffb732',
